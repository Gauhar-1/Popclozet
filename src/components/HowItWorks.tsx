...
const HowItWorks = () => {
  ...
  return (
    <section
      ref={sectionRef}
      id="how-it-works"
      className="relative py-20 md:py-32 overflow-hidden bg-gradient-to-br from-[#d4a2b1]/20 via-white to-[#d4a2b1]/10"
    >
      ...
      <div className="container mx-auto px-4 relative z-10 max-w-6xl">
        ...
        {/* Editorial Header */}
        <div className="header-anim text-center mb-16 md:mb-24 opacity-0 translate-y-[50px]"> 
        ...
        {/* Steps Grid */}
        <div className="steps-container grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 relative">
          {steps.map((step, index) => {
            ...
            return (
              <div
                ...
                className="step-card opacity-0 group relative bg-white rounded-[2rem] p-8 text-center shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-white/50 ring-1 ring-[#d4a2b1]/20"
              >
                {/* Step Badge Pill */}
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#8B1A3D] text-white px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-md group-hover:bg-[#1a1a1a] transition-colors">
                  {step.badge}
                </div>

                {/* Icon Container */}
                <div className="w-20 h-20 mx-auto mb-6 bg-[#d4a2b1]/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-500 ease-out relative overflow-hidden">
                   <div className="absolute inset-0 bg-[#8B1A3D] opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-full" />
                   <Icon className="w-8 h-8 text-[#8B1A3D] relative z-10 group-hover:text-white transition-colors duration-500" strokeWidth={1.5} />
                </div>

                {/* Content */}
                <h3 className="text-xl md:text-2xl font-serif font-bold text-gray-900 mb-3 group-hover:text-[#8B1A3D] transition-colors">
                  {step.title}
                </h3>
